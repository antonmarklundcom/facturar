...
import { fetchPublicUrl } from "@/lib/net/safe-fetch";
 ...
 *
 * The URL comes from a tenant admin, and PDF rendering is reachable from the
 * *public* buyer route, so the fetch goes through `fetchPublicUrl` — https
 * only, resolved address must be public, no redirects. See the note there:
 * without it this is a plain SSRF into the hosting network.
 */
export async function fetchLogoDataUri(logoUrl: string | null): Promise<string | null> {
  try {
    const response = await fetchPublicUrl(logoUrl, {
